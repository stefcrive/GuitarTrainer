...
export async function PUT(request: NextRequest) {
  ...
  const { positionMs, deviceId } = body

  if (typeof positionMs !== 'number' || positionMs < 0) {
    return NextResponse.json({ error: 'positionMs (>=0) is required.' }, { status: 400 })
  }

  const url = new URL('https://api.spotify.com/v1/me/player/seek')
  url.searchParams.set('position_ms', `${Math.round(positionMs)}`)
  if (deviceId) {
